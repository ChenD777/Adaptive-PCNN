...
from mlp.module import PC_MLP, Adapt_PC_MLP
from mlp.data import prepare_data
from mlp.util import model_save_name_factory, format_elapsed_time, inverse_normalize, inverse_standardize
...
class Model:
    ...
    def __init__(self, data: pd.DataFrame, interval: int, model_kwargs: dict, inputs_D: list, 
        ...
        """
        assert module in ['PC_MLP', 'Adapt_PC_MLP'],\
            f"The provided model type {module} does not exist, please chose among `'PC_MLP', 'Adapt_PC_MLP'`."
        ...
        self.mlp_hidden_size = model_kwargs["mlp_hidden_size"]
        self.mlp_num_layers = model_kwargs["mlp_num_layers"]
        ...
        elif self.module == "Adapt_PC_MLP":
            self.model = Adapt_PC_MLP(
                device=self.device,
                supply_T_column=self.supply_T_column,
                supply_m_column=self.supply_m_column,
                power_column=self.power_column,
    ...
    def predict(self, sequences: Union[list, int] = None, data: torch.Tensor = None, mpc_mode: bool = False):
        ...
        for i in range(batch_x.shape[1]):
            ...
            pred = self.model(batch_x[:, i, :],  warm_start=i<self.warm_start_length)
            predictions[:, i, :] = pred
    ...
    def load(self, load_last: bool = False):
        ...
        try:
            ...
            self.mlp_hidden_size = checkpoint["mlp_hidden_size"]
            self.mlp_num_layers = checkpoint["mlp_num_layers"]
