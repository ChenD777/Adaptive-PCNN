...
from lstm.parameters import DATA_SAVE_PATH
...
def normalize(data: pd.DataFrame):
    """
    Function to normalize the columns of a DataFrame to 0.0-1.0
    ...
    # Scale the data between 0.0 and 1.0
    data.iloc[:, non_zero_div] = (data.iloc[:, non_zero_div] - min_[non_zero_div]) / (max_[non_zero_div] - min_[non_zero_div])
...
def inverse_normalize(data: Union[np.ndarray, pd.DataFrame, float], min_: Union[pd.Series, float], max_: Union[pd.Series, float]):
    """
    Function to inverse the normalization of the columns of a DataFrame to 0.0-1.0
    ...
    if isinstance(data, float):
        ...
        if isinstance(data, np.ndarray):
            # Can get back to the original scale back
            data[:, non_zero_div] = data[:, non_zero_div] * (max_[non_zero_div] - min_[non_zero_div]).values.reshape(1, -1)
            data[:, non_zero_div] = data[:, non_zero_div] + min_[non_zero_div].values.reshape(1, -1)

        # If the given data is a DataFrame
        elif isinstance(data, pd.DataFrame):
            # Can get back to the original scale back
            data.iloc[:, non_zero_div] = data.iloc[:, non_zero_div].multiply(max_[non_zero_div] - min_[non_zero_div])
...
def model_save_name_factory(rooms, module, model_kwargs):
    ...
    for room in rooms:
        name += f"{room}_"
    name = os.path.join(model_kwargs["save_path"], f"{model_kwargs['seed']}_"f"{module}_{name}")

    name += f"{model_kwargs['overlapping_distance']}_{model_kwargs['maximum_sequence_length']}"
